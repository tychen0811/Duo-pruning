...
bool TreeClassification::prePruning(size_t nodeID,uint prepruningNumber){

  if(prepruningNumber == 1){
     //Check node size, stop if maximum reached
    if(sampleIDs[nodeID].size() <= min_node_size){
      // split_values[nodeID] = estimate(nodeID);
     /* printf("-------------------------%d--\n",sampleIDs[nodeID].size());
      printf("--------------min_node_size--%d\n",min_node_size);*/
      return true;
    }
    else
    return false;
...
bool TreeClassification::splitNodeInternal(size_t nodeID, std::vector<size_t>& possible_split_varIDs,uint prepruning) {

  if(prepruning != 0){
     if(prePruning(nodeID,prepruning))
          node_light++;
     else
          node_normal++;
  }
