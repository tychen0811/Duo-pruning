...
public:
  ...
  void init(Data* data, Data* validation_data, Data* test_data, uint mtry, size_t dependent_varID, size_t num_samples, uint seed,
  ...
  void predict(const Data* prediction_data, bool oob_prediction);  void computePermutationImportance(std::vector<double>* forest_importance, std::vector<double>* forest_variance);
...
protected:
  ...
  virtual bool splitNodeInternal(size_t nodeID, std::vector<size_t>& possible_split_varIDs,uint prepruning) = 0;
...
//  bool check_pruning(size_t nodeID);
  void record_trigger_node(size_t nodeID);
  ...
  size_t node_normal;
  size_t node_light;
  std::vector<size_t> trigger_node;
  ...
  //prepruning split values
  std::vector<double> prepruning_split_values;
  // Vector of left and right child node IDs, 0 for no child
  std::vector<std::vector<size_t>> child_nodeIDs;
  // prepruning vector
  std::vector<std::vector<size_t>> prepruning_child_nodeIDs;
