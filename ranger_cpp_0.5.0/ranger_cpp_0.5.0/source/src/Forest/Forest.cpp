...
Forest::Forest() :
    verbose_out(0), num_trees(DEFAULT_NUM_TREE), mtry(0), min_node_size(0), num_variables(0), num_independent_variables(
        ...
        1), holdout(false), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), num_threads(DEFAULT_NUM_THREADS), data(0), validation_data(0), test_data(0), overall_prediction_error(
...
void Forest::initCpp(std::string dependent_variable_name, MemoryMode memory_mode, std::string input_file, std::string validation_file, std::string test_file, uint mtry,
  ...
  // Initialize validation data with memmode
  switch(memory_mode) {
  case MEM_DOUBLE:
  ...
  // Initialize test data with memmode
  switch(memory_mode) {
  case MEM_DOUBLE:
   test_data = new DataDouble();
   break;
  case MEM_FLOAT:
   test_data = new DataFloat();
   break;
  case MEM_CHAR:
   test_data = new DataChar();
   break;
  }
  ...
  *verbose_out << "Loading validation file: " << validation_file << "." << std::endl;
  ...
  // Load test  data
  *verbose_out << "Loading test file: " << test_file << "." << std::endl;
  bool rounding_error3 = test_data->loadFromFile(test_file);
  if (rounding_error3) {
    *verbose_out << "Warning: Rounding or Integer overflow occurred. Use FLOAT or DOUBLE precision to avoid this."
        << std::endl;
  }
  ...
  init(dependent_variable_name, memory_mode, data, validation_data, test_data, mtry, output_prefix, num_trees, seed, num_threads, importance_mode,
...
void Forest::initR(std::string dependent_variable_name, Data* input_data, uint mtry, uint num_trees,
  ...
  // Call other init function
  init(dependent_variable_name, MEM_DOUBLE, input_data, validation_data,test_data, mtry, "", num_trees, seed, num_threads, importance_mode,
      min_node_size, status_variable_name, prediction_mode, sample_with_replacement, unordered_variable_names,
      memory_saving_splitting, splitrule, predict_all, sample_fraction, alpha, minprop, holdout, prepruning, postpruning);
...
void Forest::init(std::string dependent_variable_name, MemoryMode memory_mode, Data* input_data, Data*input_data2, Data* input_data3, uint mtry,
    std::string output_prefix, uint num_trees, uint seed, uint num_threads, ImportanceMode importance_mode,
    uint min_node_size, std::string status_variable_name, bool prediction_mode, bool sample_with_replacement,
    std::vector<std::string>& unordered_variable_names, bool memory_saving_splitting, SplitRule splitrule,
    bool predict_all, double sample_fraction, double alpha, double minprop, bool holdout,uint prepruning,uint postpruning) {

  // Initialize data with memmode
  this->data = input_data;
  this->validation_data = input_data2;
  this->test_data = input_data3;
...
void Forest::grow() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    trees[i]->init(data,validation_data,test_data,mtry, dependent_varID, num_samples, tree_seed, &deterministic_varIDs, &split_select_varIDs,
