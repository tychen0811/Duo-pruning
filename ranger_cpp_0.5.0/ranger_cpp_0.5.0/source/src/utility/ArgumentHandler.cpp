...
ArgumentHandler::ArgumentHandler(int argc, char **argv) :
    caseweights(""), depvarname(""), fraction(1), holdout(false), memmode(MEM_DOUBLE), savemem(false), predict(""), splitweights(
        ""), nthreads(DEFAULT_NUM_THREADS), predall(false), alpha(DEFAULT_ALPHA), minprop(DEFAULT_MINPROP), file(""), file_validation(""), file_test(""), impmeasure(
...
int ArgumentHandler::processArguments() {
  ...
  // short options
  char const *short_options = "A:C:D:F:HM:NP:S:U:XZa:b:c:e:f:V:T:hil::m:o:pr:s:t:uvwx:y:z:";
  ...
  // long options: longname, no/optional/required argument?, flag(not used!), shortname
    const struct option long_options[] = {
      ...
      { "file_test",            required_argument,  0, 'T'},
  ...
  while (1) {
    ...
    case 'e':
	 ...
	 postpruning = temp;
      ...
      throw std::runtime_error(
             "Illegal argument for option 'postpruning'. Please give a positive integer. See '--help' for details.");
      }
      break;
    ...
    case 't':
      try {
        ...
        throw std::runtime_error("Illegal argument for option 'ntree'. Please give a positive integer. See '--help' for details.");
      }
      break;
    ...
    case 'x':
       ...
       throw std::runtime_error("Illegal argument for option 'prepruning'. Please give a positive integer. See '--help' for details.");
        }     
      break; 
